...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description='Mutation testing for C source.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.list:
            payload = {"schema_version": 1, "tool": 'mutate4c', "version": __version__,
        ...
        if 'c' in {"c", "cpp", "objc"} and not args.no_validate and not validation_command:
        ...
        write_manifest(manifest, 'mutate4c', __version__, root, results)
    except (OSError, ValueError, MutationError, json.JSONDecodeError) as error:
        print(f"mutate4c: {error}", file=sys.stderr)
        return 1
